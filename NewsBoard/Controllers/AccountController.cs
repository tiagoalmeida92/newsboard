...
namespace NewsBoard.Web.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        //NewsBoard Edit - Error on AuthenticationManager.GetExternalLoginInfoAsync() - so created new method to replace
        private async Task<ExternalLoginInfo> AuthenticationManager_GetExternalLoginInfoAsync_Workaround()
        {
            ...
            {
                Claim idClaim = result.Identity.FindFirst(ClaimTypes.NameIdentifier);
                if (idClaim != null)
                {
        ...
        {
            //CHANGED TO AuthenticationManager_GetExternalLoginInfoAsync_Workaround from AuthenticationManager.GetExternalLoginInfoAsync()
            ExternalLoginInfo loginInfo = await AuthenticationManager_GetExternalLoginInfoAsync_Workaround();
            if (loginInfo == null)
            {
            ...
            string returnUrl)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                //CHANGED TO AuthenticationManager_GetExternalLoginInfoAsync_Workaround from AuthenticationManager.GetExternalLoginInfoAsync()
                ExternalLoginInfo info = await AuthenticationManager_GetExternalLoginInfoAsync_Workaround();
                if (info == null)
                {
        ...
        [HttpPost]
        public ActionResult UpdateIgnoredNewsSources(IEnumerable<int> ignored)
        {
            if (ignored == null) ignored = Enumerable.Empty<int>();
            ApplicationUser user = UserManager.FindByName(User.Identity.Name);
            user.IgnoredNewsSourcesIds = ignored;
        ...
        //NewsBoard Edit - ADDED from src: http://www.theroks.com/social-login-owin-authentication-mvc5/
        private async Task SaveAccessToken(ApplicationUser user, ClaimsIdentity identity)
        {
            IList<Claim> userclaims = await UserManager.GetClaimsAsync(user.Id);
        ...
        private async Task SetExternalProperties(ClaimsIdentity identity)
        {
            ...
            if (ext != null)
            {
                string ignoreClaim = "http://schemas.xmlsoap.org/ws/2005/05/identity/claims";
                // add external claims to identity
                foreach (Claim c in ext.Claims)
                {
        ...
        //NewsBoard Edit - EDITED
        /* 
         * Saving the access tokens let us in depended off the user, access their information. 
         * This can be useful, but be aware of the privacy rules that apply on the application.
         */
